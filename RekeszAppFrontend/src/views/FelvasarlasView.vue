...
const showPartnerModal=ref(false),showVevoModal=ref(false),showZoldsegModal=ref(false),showRekeszModal=ref(false),editId=ref(null),editForm=ref({})
const vevek=ref([])
...
async function torzsadatok(){try{const[p,v,z,r]=await Promise.all([client.get('/partnerek'),client.get('/vevek'),client.get('/zoldsegek'),client.get('/rekesztipusok')]);partnerek.value=p.data;vevek.value=v.data;zoldsegek.value=z.data;rekesztipusok.value=r.data}catch{hiba.value='Nem sikerült betölteni a törzsadatokat.'}}
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="torzsadat-linkek">
        <button type="button" class="link-btn" @click="showPartnerModal=true">Eladók kezelése</button><button type="button" class="link-btn" @click="showVevoModal=true">Vevők kezelése</button>
        ...
        <button v-for="group in csoportositottFelvasarlas" :key="group.key" type="button" class="felv-tile" @click="nyitFelvCsoportDetail(group)">
          <div class="felv-tile-photo" :class="{'no-photo':!group.zoldsegKepUrl}" :style="csempeStilus(group.zoldsegId)">
            <div class="felv-tile-overlay">
              ...
              <span class="felv-tile-info">{{group.mennyiseg}} db · {{group.rekeszTipus}} rekesz · {{group.egysegar!=null?Number(group.egysegar).toLocaleString('hu-HU')+' Ft/db':'ár nélkül'}}</span>
        ...
        <button type="button" class="felv-tile" @click="nyitRaktarDetail(s)">
          <div class="felv-tile-photo" :class="{'no-photo':!s.zoldsegKepUrl}" :style="csempeStilus(s.zoldsegId)">
            <div class="felv-tile-overlay">
              ...
              <span class="felv-tile-info">{{s.maradt}} db · {{s.rekeszTipus}} rekesz</span>
...
.raktar-card{margin-top:18px}.raktar-hint{margin:-6px 0 12px}.raktar-tile-wrap{display:flex;flex-direction:column}.athelyez-btn{margin-top:6px;font-size:12px;padding:7px 10px;border-radius:8px;background:var(--paper);border:1px solid rgba(43,58,46,.2);color:var(--chalk-green);cursor:pointer}
.felv-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px}.felv-tile{text-align:left;border:1px solid var(--line);border-radius:12px;background:var(--paper);overflow:hidden;cursor:pointer;padding:0;display:flex;flex-direction:column;box-shadow:var(--shadow-soft)}.felv-tile-photo{position:relative;aspect-ratio:16/9;background-color:rgba(107,122,79,.08);background-size:cover;background-position:center;display:flex;align-items:flex-end;justify-content:center}.felv-tile-placeholder{font-size:30px;margin-bottom:10px}.felv-tile-overlay{position:absolute;top:0;left:0;right:0;background:var(--paper);display:flex;flex-direction:column}.felv-tile-name{padding:7px 10px 2px;font-size:15px;font-weight:800;color:var(--brand-deep);text-transform:none}.felv-tile-info{padding:0 10px 7px;font-size:13px;font-weight:600;color:var(--muted);text-transform:none}
.felv-detail-modal,.felv-csoport-modal{max-width:460px}.overlay{position:fixed;inset:0;background:rgba(35,38,32,.55);display:flex;align-items:center;justify-content:center;z-index:60;padding:14px}.athelyez-modal{width:100%;max-width:380px}.nav-row{display:flex;gap:10px;margin-top:10px}.nav-row button{flex:1;min-height:40px;border-radius:8px;font-weight:700;cursor:pointer}.btn-primary{background:var(--chalk-green);color:var(--paper);border:0}.btn-secondary{background:var(--paper);border:1px solid rgba(43,58,46,.2);color:var(--chalk-green)}.narrow-3{max-width:70px}.narrow-5{max-width:95px}.foto-row{margin:-2px 0 14px}.foto-btn{width:100%;min-height:44px;border-radius:10px;border:2px dashed rgba(43,58,46,.3);background:rgba(107,122,79,.05);color:var(--chalk-green);font-weight:700;font-size:13px;cursor:pointer;text-transform:none}.foto-btn:hover{border-color:var(--olive)}.foto-btn:disabled{opacity:.6;cursor:default}
.toggle-pill{display:inline-flex;align-items:center;gap:6px;border:1px solid rgba(43,58,46,.2);border-radius:20px;padding:9px 16px;background:var(--paper);color:var(--chalk-green);font-weight:700;font-size:13px;cursor:pointer;margin-bottom:14px}.toggle-pill.active{background:var(--chalk-green);color:var(--paper);border-color:var(--chalk-green)}.torzsadat-linkek{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px;padding-top:12px;border-top:1px solid rgba(43,58,46,.1)}.link-btn{background:var(--paper);border:1px solid rgba(43,58,46,.25);border-radius:8px;padding:8px 12px;font-size:12px;font-weight:700;color:var(--chalk-green);cursor:pointer;text-transform:none}.link-btn:hover{background:rgba(107,122,79,.08);border-color:var(--olive)}.raktar-detail-modal{max-width:480px}.raktar-detail-hint{font-size:12px;line-height:1.45}.forras-list{display:flex;flex-direction:column;gap:8px;margin-top:12px}.forras-row{display:flex;justify-content:space-between;align-items:center;gap:12px;border:1px solid rgba(43,58,46,.12);border-radius:9px;padding:10px;background:var(--paper)}.forras-row strong{color:var(--chalk-green);font-size:13px}.forras-row .muted{font-size:11px;margin-top:3px}.forras-btn{white-space:nowrap;padding:7px 10px;border-radius:8px;font-weight:700;cursor:pointer}
</style>

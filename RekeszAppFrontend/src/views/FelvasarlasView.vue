        ...
        <button v-for="group in csoportositottFelvasarlas" :key="group.key" type="button" class="felv-tile" @click="nyitFelvCsoportDetail(group)">
          <div class="felv-tile-photo" :class="{'no-photo':!group.zoldsegKepUrl}" :style="csempeStilus(group.zoldsegId)">
            <div class="felv-tile-overlay">
              ...
              <span class="felv-tile-info">{{group.mennyiseg}} db · {{group.egysegar!=null?ft(group.egysegar)+' Ft/db':'ár nélkül'}}</span>
              <span class="felv-tile-info">Rekesz: {{group.rekeszTipus}}<template v-if="group.items.length>1"> · {{group.items.length}} tétel</template></span>
            </div>
        ...
        <button type="button" class="felv-tile" @click="nyitRaktarDetail(s)">
          <div class="felv-tile-photo" :class="{'no-photo':!s.zoldsegKepUrl}" :style="csempeStilus(s.zoldsegId)">
            <div class="felv-tile-overlay">
              ...
              <span class="felv-tile-info">{{s.maradt}} db · Rekesz: {{s.rekeszTipus}}</span>
              <span v-if="s.arNelkulMennyiseg>0" class="felv-tile-info">Ebből {{s.arNelkulMennyiseg}} db ár nélkül</span>
            </div>
...
.raktar-card{margin-top:18px}.raktar-hint{margin:-6px 0 12px}.raktar-tile-wrap{display:flex;flex-direction:column}.athelyez-btn{margin-top:6px;font-size:12.5px;font-weight:700;padding:8px 10px;border-radius:8px;background:var(--paper);border:1px solid var(--line-strong);color:var(--chalk-green);cursor:pointer}
.felv-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px}.felv-tile{text-align:left;border:1px solid var(--line);border-radius:12px;background:var(--paper);overflow:hidden;cursor:pointer;padding:0;display:flex;flex-direction:column;box-shadow:var(--shadow-soft)}.felv-tile-photo{position:relative;aspect-ratio:16/9;background-color:var(--surface-soft);background-size:cover;background-position:center;display:flex;align-items:flex-end;justify-content:center}.felv-tile-placeholder{font-size:30px;margin-bottom:10px}.felv-tile-overlay{position:absolute;top:0;left:0;right:0;padding:7px 9px;display:flex;flex-direction:column;gap:2px}.felv-tile-name{font-size:15px;font-weight:800;line-height:1.2;color:var(--tile-overlay-ink);text-transform:none;overflow-wrap:anywhere}.felv-tile-info{font-size:12.5px;font-weight:600;line-height:1.3;color:var(--tile-overlay-muted);text-transform:none}
